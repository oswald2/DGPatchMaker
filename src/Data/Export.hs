...
conduitSamples ss = foldr f mempty ss
  where
    f x b =
        tag
                "sample"
                (  attr "name"  (hsName x)
                <> attr "power" ((pack . show . hsPower) x)
                )
...
conduitAudioFiles afs = foldr f mempty afs
  where
    f x b =
        tag
                "audiofile"
                (  attr "channel"     (afChannel x)
                <> attr "file"        (pack (afPath x))
                <> attr "filechannel" ((pack . show . afFileChannel) x)
                )
                mempty
            <> b
