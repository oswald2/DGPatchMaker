...
instrumentPageNew
  ...
  -> (Int -> IO ())
  -> IO InstrumentPage
instrumentPageNew parentWindow notebook basedir samplesDir combo ioref fhDialog errDiag updateSampleRate
  = do
    ...
    menuAddHitSample <- builderGetObject builder
                                         castToMenuItem
    ...
    btInstrPath <- builderGetObject builder
                                    castToButton
                                    ("buttonInstrumentPath" :: Text)
    entryInstrPath <- builderGetObject builder
                                       castToEntry
                                       ("entryInstrumentPath" :: Text)
    ...
    let gui = InstrumentPage { guiInstMainBox          = mainBox
                             ...
                             , guiInstPath             = entryInstrPath
                             , guiButtonInstPath       = btInstrPath
                             }
    ...
    void
      $ on entryName focusOutEvent (liftIO (validateName gui) >> return False)
    void $ on entryType entryActivated (validateType gui)
    void
      $ on entryType focusOutEvent (liftIO (validateType gui) >> return False)
...
getAudioSamplesFromFiles
  :: (Int -> IO ()) -> FilePath -> [FilePath] -> IO [AudioFile]
...
initTreeViewHit
  :: TreeView -> ListStore HitSample -> IO (CellRendererText, CellRendererText)
...
initTreeViewSamples
  :: TreeView -> ListStore AudioFile -> IO (CellRendererCombo, CellRendererText)
...
getInstrumentFromGUI instPage = do
  ...
  fpath   <- entryGetText (guiInstPath instPage)
  ...
  case t' of
    Left _ -> do
      let res = InstrumentFile version name fpath fname Nothing samples
      return (Right res)
    Right typ -> do
      let res = InstrumentFile version name fpath fname (Just typ) samples
      return (Right res)
  ...
  case s of
    ((i : _) : _) -> do
      ...
      let filech = afFileChannel val
          indexes =
            P.map fst . P.filter ((== filech) . afFileChannel . snd) $ lst
...
convertToFullMix gui = do
  ...
  case i of
    Just iF -> do
      let
        newSamples = P.map toStereo (ifSamples iF)
        newIF      = iF { ifFileName = newName, ifSamples = newSamples }
        newName =
          pack . (<> "FL.xml") . dropExtension . unpack . ifFileName $ iF
...
setLinearHitPower gui = do
  ...
  case res of
    ...
    Just (HitPowerDialogResult start stop step) -> do
      ...
      setListStoreTo (guiInstHitViewModel gui)
        $ P.zipWith (\x y -> x { hsPower = fromIntegral y }) lst values
